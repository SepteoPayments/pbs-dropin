...
async function submitWithCountry(
	result: { current: { updateField: (key: 'shopperCountryCode', value: string) => void; submit: () => Promise<void> } }
) {
	act(() => {
...
describe('usePbsDropinFlow', () => {
	...
	it('freezes a single locale and hides the selector', () => {
		const { result } = renderHook(() => usePbsDropinFlow(dropinTestProps({ locales: ['es-ES'] })))
	...
	it('uses the locale prop as the Adyen locale by default', () => {
		const { result } = renderHook(() => usePbsDropinFlow(dropinTestProps({ locale: 'en-US' })))
	...
	it('sends synthetic lineItems when Klarna / BNPL is enabled', async () => {
		createSessionMock.mockResolvedValue({
			sessionRequestId: 'req',
			status: 'CREATED',
			publicSessionId: 'pub',
		...
		act(() => {
			result.current.updateField('includeLineItems', true)
			result.current.updateField('shopperCountryCode', 'FR')
	...
	it('sends the lineItems prop on session create', async () => {
		...
		const { result } = renderHook(() => usePbsDropinFlow(dropinTestProps({ lineItems })))
