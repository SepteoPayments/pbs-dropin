...
This is an early `0.2.0` release. Field layout and product rules may still change.
...
Klarna / BNPL only appear in the Adyen session when `lineItems` **and** `shopperCountryCode` are sent. Pass a real basket via the `lineItems` prop. Shopper country is always required so Adyen can filter methods (SEPA, etc.).
